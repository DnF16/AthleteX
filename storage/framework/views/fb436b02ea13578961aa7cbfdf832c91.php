...
<div class="p-8 relative z-10">
    ...
    <?php else: ?>
        <div class="bg-red-600 text-white p-6 rounded-lg shadow-lg mb-8 flex items-center justify-between animate-pulse">
            <div>
                ...
                <p class="text-red-100">The data integrity ledger is compromised at Block(s): <strong class="text-white"><?php echo e(implode(', ', $tamperedBlocks)); ?></strong>. Manual database modification detected!</p>
            </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="bg-white p-6 rounded-lg shadow border-l-4 border-[#2e4e1f]">
            ...
            <p class="text-4xl font-black text-[#2e4e1f]"><?php echo e($totalBlocks); ?></p>
        </div>
        
        <div class="bg-white p-6 rounded-lg shadow border-l-4 <?php echo e($isSecure ? 'border-green-500' : 'border-red-600'); ?>">
            <p class="text-gray-500 text-xs font-bold tracking-wider mb-1">COMPROMISED RECORDS</p>
            <p class="text-4xl font-black <?php echo e($isSecure ? 'text-green-500' : 'text-red-600'); ?>"><?php echo e(count($tamperedBlocks)); ?></p>
        </div>

        <div class="bg-white p-6 rounded-lg shadow border-l-4 border-blue-500">
            <p class="text-gray-500 text-xs font-bold tracking-wider mb-1">LAST VERIFICATION SCAN</p>
            <p class="text-2xl font-bold text-blue-600 mt-2"><?php echo e(now()->format('h:i:s A')); ?></p>
        </div>
    </div>
                ...
                <tbody class="text-sm">
                    <?php $__currentLoopData = $logs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $log): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr class="hover:bg-gray-50 border-b">
                            <td class="p-4 font-bold text-gray-700">#<?php echo e($log->id); ?></td>
                            <td class="p-4 text-gray-500"><?php echo e($log->created_at->format('M d, Y - h:i A')); ?></td>
                            <td class="p-4 font-semibold text-[#2e4e1f]"><?php echo e($log->user ? $log->user->name : 'System/Guest'); ?></td>
                            <td class="p-4"><?php echo e($log->action); ?></td>
                            <td class="p-4 font-mono text-xs text-gray-400" title="<?php echo e($log->current_hash); ?>">
                                <?php echo e(substr($log->current_hash, 0, 16)); ?>...
                            </td>
                            <td class="p-4 text-center">
                                <?php if(in_array($log->id, $tamperedBlocks)): ?>
                            ...
                            <td class="p-4 text-center">
                                ...
                                <button type="button" 
                                        ...
                                        data-block="<?php echo e($log->id); ?>"
                                        data-action="<?php echo e($log->action); ?>">
                                    View Data
                                </button>
                                
                                <!-- The Hidden Data Vault (Prevents HTML breakage) -->
                                <textarea id="payload-<?php echo e($log->id); ?>" class="hidden"><?php echo $log->payload ?? $log->details ?? $log->data ?? '{}'; ?></textarea>
                            </td>
                        </tr>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\xampp\htdocs\AthleteX\resources\views/features/blockchain_dashboard.blade.php ENDPATH**/ ?>