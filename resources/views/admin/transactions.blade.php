...
<div class="container-fluid p-0"><div class="row g-0">
    <div class="col-md-3 col-lg-2 p-3" style="background-color: #dbead5; min-height: 100vh; border-right: 1px solid #c4d79b;">@include('admin.partials.sidebar')</div>
    <div class="col-md-9 col-lg-10 p-4" style="background-color: #EBF1DE;">
        ...
        <div class="row">
            <div class="col-md-4"><div class="section-box"><h6 class="fw-bold border-bottom pb-2">Add Item</h6>
                <form action="{{ route('admin.addTransaction') }}" method="POST">@csrf
            ...
            <div class="col-md-8"><table class="table table-bordered custom-table bg-white table-striped">
                ...
                <tbody>@foreach($items as $item)<tr><td>{{ $item->name }}</td><td>{{ $item->type }}</td><td>{{ $item->category }}</td></tr>@endforeach</tbody>
            </table></div>
        </div>
    </div>
</div></div>
