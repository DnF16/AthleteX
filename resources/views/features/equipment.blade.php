                    ...
                    <table class="table table-hover align-middle mb-0 w-100">
                        ...
                        <tbody>
                            @foreach($requests as $req)
                                <tr>
                                    <td class="ps-4 fw-semibold text-dark">{{ \Carbon\Carbon::parse($req->date_requested)->format('M d, Y') }}</td>
                                    <td class="text-dark fw-bold uppercase">{{ $req->requested_by }}</td>
                                    <td class="text-secondary">{{ $req->event }}</td>
                                    <td>
                                        @php
                                            $itemCount = is_array($req->items) ? count($req->items) : (is_string($req->items) ? count(json_decode($req->items, true) ?? []) : 0);
                                        @endphp
                                        <span class="badge bg-secondary rounded-pill px-3">{{ $itemCount }} items</span>
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-center">
                                        <button class="btn btn-sm btn-outline-secondary fw-bold shadow-sm" data-bs-toggle="modal" data-bs-target="#viewRequestModal{{ $req->id }}">
                    ...
                    <div class="bg-white mx-auto my-4 p-5 shadow-sm" style="max-width: 900px; border: 1px solid #dee2e6;">
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-4 d-flex align-items-center">
                                ...
                                <input type="date" name="date_requested" class="form-control px-2 flex-grow-1" value="{{ date('Y-m-d') }}" required style="background: transparent; border: none; border-bottom: 1px solid black; border-radius: 0; box-shadow: none;">
                            </div>
                        </div>
                        ...
                        <div class="row mt-4 pt-4">
                            <div class="col-md-6">
                                ...
                                <div class="px-4 text-center" style="margin-top: 60px;">
                                    <input type="text" name="requested_by" class="form-control text-center fw-bold text-uppercase px-2 mb-1" value="{{ auth()->user()->role === 'coach' ? (auth()->user()->coach ? auth()->user()->coach->coach_first_name . ' ' . auth()->user()->coach->coach_last_name : auth()->user()->name) : auth()->user()->name }}" required style="background: transparent; border: none; border-bottom: 1px solid black; border-radius: 0; box-shadow: none;">
...
@if(isset($requests) && !$requests->isEmpty())
    @foreach($requests as $req)
        <div class="modal fade" id="viewRequestModal{{ $req->id }}" tabindex="-1" data-bs-backdrop="false" style="background-color: rgba(0, 0, 0, 0.6);">
            <div class="modal-dialog modal-xl modal-dialog-centered">
                <div class="modal-content border-0 shadow-lg" style="border-radius: 4px;">
                        ...
                        <div class="bg-white mx-auto my-4 p-5 shadow-sm text-dark" style="max-width: 900px; border: 1px solid #dee2e6;">
                            ...
                            <div class="row mb-4">
                                ...
                                <div class="col-md-4 d-flex align-items-center">
                                    <label class="fw-bold me-2 mb-0">Date:</label>
                                    <div class="flex-grow-1 px-2 pb-1" style="border-bottom: 1px solid black;">
                                        {{ \Carbon\Carbon::parse($req->date_requested)->format('m/d/Y') }}
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <tbody>
                                        ...
                                        @if($items)
                                            @foreach($items as $item)
                                                @php $totalAmount += floatval($item['amount'] ?? 0); @endphp
                                                <tr class="align-middle text-center">
                                                    <td>{{ $item['qty'] ?? 0 }}</td>
                                                    <td>{{ $item['unit'] ?? '' }}</td>
                                                    <td class="text-start px-3">{{ $item['description'] ?? '' }}</td>
                                        ...
                                        <tr class="align-middle fw-bold bg-light">
                                            ...
                                            <td class="text-center text-success">₱ {{ number_format($totalAmount, 2) }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                                    ...
                                    <div class="px-4 text-center" style="margin-top: 60px;">
                                        ...
                                        @if($req->status === 'Approved')
                                            <div class="fw-bold text-success px-2 mb-1 pb-1 d-flex justify-content-center align-items-center" style="border-bottom: 1px solid black; font-family: 'Courier New', Courier, monospace;">
                                                <i class="fas fa-check-double me-2"></i> DIGITAL APPROVAL ({{ $req->updated_at->format('m/d/Y') }})
                                            </div>
                            ...
                            <div>
                                @if(auth()->user()->role === 'admin' && $req->status === 'Pending')
                                    <div class="d-flex gap-2">
                                        <form action="{{ route('equipment.approve', $req->id) }}" method="POST">
                                            @csrf
                                        ...
                                        <form action="{{ route('equipment.reject', $req->id) }}" method="POST">
                                            @csrf
