    ...
    public function index(\Illuminate\Http\Request $request)
{
    if (auth()->check() && auth()->user()->role === 'coach') {
        ...
        // Load ALL relationships consistently - FIX: use workHistories plural
        $withRelations = ['achievements', 'workHistories', 'memberships', 'schedule', 'expenses', 'seminars'];
        
        if ($requestedCoachId && $user->coach_id == $requestedCoachId) {
            $coach = Coach::with($withRelations)->find($requestedCoachId);
        } elseif ($user->coach_id) {
            $coach = Coach::with($withRelations)->find($user->coach_id);
        } else {
            $coach = $user->coach ? $user->coach->load($withRelations) : null;
        }
...
{
    $payload = $request->all();
    $general = $payload['generalInfo'] ?? $payload;
    ...
    try {
        $coach = DB::transaction(function () use ($general, $payload) {
            $coach = Coach::create($general);

            // ACHIEVEMENTS
            if (!empty($payload['achievements']) && is_array($payload['achievements'])) {
                foreach ($payload['achievements'] as $ach) {
                    $coach->achievements()->create([
                        'year' => $ach['year'] ?? null,
            ...
            // WORK HISTORY - FIX: Use plural workHistories and $coach->id
            if (!empty($payload['workHistories']) && is_array($payload['workHistories'])) {
                foreach ($payload['workHistories'] as $w) {
        ...
        if (auth()->user()->role === 'coach' && !auth()->user()->coach_id) {
            auth()->user()->update([
                'coach_id' => $coach->id,
                'coach_sport' => $coach->coach_sport_event
            ]);
        }
        ...
        return response()->json([
            ...
            'coach' => $coach->load(['achievements', 'workHistories', 'memberships', 'schedule', 'expenses', 'seminars'])
        ], 201);
    } catch (\Exception $e) {
        ...
        return response()->json(['success' => false, 'message' => 'Failed to create coach: ' . $e->getMessage()], 500);
    }
}
